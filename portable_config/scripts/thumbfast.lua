...
if mpv_path == "mpv" and os_name == "windows" then
    ...
    -- DinaPlayer: newer mpv builds no longer set frontend/process-path, which left
    -- thumbfast trying to spawn bare "mpv" (not on PATH) -> "cannot create mpv
    -- subprocess". Fall back to the bundled exe next to portable_config: the
    -- portable layout is <root>/DinaPlayer.exe with config at <root>/portable_config.
    if not frontend_path then
        local config_dir = mp.command_native({"expand-path", "~~/"})
        if config_dir then
            local root = config_dir:gsub("[/\\]+$", ""):gsub("[/\\][^/\\]+$", "")
            local exe = root .. "\\DinaPlayer.exe"
            if mp.utils.file_info(exe) then frontend_path = exe end
        end
    end
