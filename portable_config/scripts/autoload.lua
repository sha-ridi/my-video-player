...
local function alphanumsort(filenames)
    ...
    for i, f in ipairs(filenames) do
        -- DinaPlayer: sort by the name WITHOUT extension, otherwise the extension
        -- dot is compared too and e.g. "name - Copy.mkv" sorts before "name.mkv"
        -- (the space in " - Copy" is < the "." of ".mkv").
        local stem = f:gsub("%.[^.]+$", "")
        tuples[i] = {stem:lower():gsub("0*(%d+)%.?(%d*)", padnum), f}
    end
