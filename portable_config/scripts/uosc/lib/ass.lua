...
function ass_mt:icon(x, y, size, name, opts)
	...
	opts.border = 0  -- DinaPlayer: flat white icons, no black outline (text keeps its border)
...
function ass_mt:tooltip(element, value, opts)
	...
	opts = opts or {}
	-- DinaPlayer: configurable, with a fullscreen-only value — a 16px label under
	-- a 52px icon read as an afterthought. Callers that pass their own size (the
	-- timeline's time and chapter tooltips) are unaffected.
	opts.size = opts.size or round(fullscreen_size('tooltip_size') * state.scale)
