...
function cursor:is_autohide_allowed()
	-- DinaPlayer: during playback the UI must hide on inactivity even when the
	-- cursor rests on the controls (YouTube-style). Stock uosc blocks autohide
	-- whenever the cursor is over an interactive element (is_dragging_prevented);
	-- we only keep that block while PAUSED. When paused the UI stays up anyway
	-- via *_persistency=paused, so this only changes the playing case.
	return options.autohide and (not self.autohide_fs_only or state.fullscreen)
		and (not self.is_dragging_prevented or not state.pause)
