...
function create_self_updating_menu_opener(opts)
	return function()
		...
		-- DinaPlayer: extra properties (opts.update_props) that should re-serialize
		-- the menu when they change (e.g. the watched/in-progress sets for the
		-- playlist, so a manual "seen" toggle refreshes the icons immediately).
		local function handle_update_prop_change() update() end

		local function cleanup_and_close()
			mp.unobserve_property(handle_list_prop_change)
			mp.unobserve_property(handle_active_prop_change)
			mp.unobserve_property(handle_update_prop_change)
			menu:close()
		end
		...
		menu = Menu:open({
			...
			item_hint_below = opts.item_hint_below, -- DinaPlayer: two-line items (name over details)
			reserve_icons = opts.reserve_icons, -- DinaPlayer: always reserve the icon column
			icon_action = opts.icon_action, -- DinaPlayer: clickable state icon fires this action
			max_visible = opts.max_visible, -- DinaPlayer: cap on visible rows before scrolling
		...
		-- DinaPlayer: open in mouse mode so the active/current entry isn't
		-- pre-selected. Pre-selection would show its row actions (move up/down,
		-- remove) on open without the mouse being there. It's still scrolled into
		-- view and keyboard navigation re-enables selection.
		end, {mouse_nav = true})
		...
		if opts.update_props then
			for _, prop in ipairs(opts.update_props) do
				mp.observe_property(prop, 'native', handle_update_prop_change)
			end
		end
	end
end
...
function create_select_tracklist_type_menu_opener(opts)
	...
	local function serialize_tracklist(tracklist)
		...
		-- DinaPlayer: optional "None" item at the top (disables the track). Used
		-- for subtitles so turning them off is an explicit choice, not a re-click.
		local none_index = nil
		if opts.none_item then
			items[#items + 1] = {
				title = t('None'),
				italic = true,
				muted = track_prop_index ~= nil,
				active = track_prop_index == nil,
				value = 'no',
				separator = true,
			}
			none_index = #items
		...
		for _, track in ipairs(tracklist) do
			if track.type == opts.type then
				...
				-- DinaPlayer: `lang_hint` replaces the details line (codec, channels,
				-- sample rate…) with just the language code on the right, matching
				-- the subtitles popup. Only when the track has a title of its own —
				-- when it has none, the language becomes the title instead (below).
				local hint = table.concat(hint_values, ', ')
				if opts.lang_hint then
					hint = (track.title and track.lang) and track.lang:upper() or nil
				end

				items[#items + 1] = {
					-- DinaPlayer: no title of its own → show the language code
					-- (matches the subtitles popup) rather than a meaningless "Track N".
					title = track.title or (track.lang and track.lang:upper()) or t('Track %s', track.id),
					hint = hint,
					value = track.id,
		...
		return items, active_index or none_index or first_item_index
	end
	...
	local function handle_activate(event)
		if event.value == '{load}' then
			...
			elseif not event.modifiers or event.modifiers == 'alt' then
				-- Re-clicking the selected track toggles it off (switches to None).
				local chosen = event.value == get_props() and 'no' or event.value
				mp.commandv('set', opts.prop, chosen)
				-- DinaPlayer: deliberate pick -> remember its language (track-memory.lua)
				if opts.prop == 'aid' then
					mp.commandv('script-message', 'dina-track-chosen', 'aid', tostring(chosen))
				end
	...
	return create_self_updating_menu_opener({
		title = opts.title,
		-- DinaPlayer: no "?" footnote hint in the track popups.
		type = opts.type,
		list_prop = 'track-list',
		item_hint_below = opts.two_line, -- DinaPlayer: two-line items (name over details)
		max_visible = opts.max_visible, -- DinaPlayer: forward the visible-rows cap
