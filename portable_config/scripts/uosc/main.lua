...
defaults = {
	...
	timeline_size = 40,
	timeline_size_fullscreen = 0, -- DinaPlayer: 0 = inherit `timeline_size`
	-- DinaPlayer: the elapsed/remaining times in the timeline's corners.
	-- `scale` multiplies the size uosc derives from the timeline height,
	-- `border` is the text outline in pixels (before display scaling), 0 = none.
	timeline_timestamp_scale = 1,
	timeline_timestamp_border = 2,
	progress = 'windowed',
	progress_size = 2,
	...
	controls_size = 32,
	controls_size_fullscreen = 0, -- DinaPlayer: 0 = inherit `controls_size`
	controls_margin = 8,
	controls_spacing = 2,
	...
	menu_item_height = 36,
	menu_item_height_fullscreen = 0, -- DinaPlayer: 0 = inherit `menu_item_height`
	menu_min_width = 260,
	menu_min_width_fullscreen = 0, -- DinaPlayer: 0 = inherit `menu_min_width`
	menu_padding = 4,
	menu_padding_fullscreen = 0, -- DinaPlayer: 0 = inherit `menu_padding`
	menu_type_to_search = true,

	top_bar = 'no-border',
	top_bar_size = 40,
	top_bar_size_fullscreen = 0, -- DinaPlayer: 0 = inherit `top_bar_size`
	top_bar_persistency = '',
	...
	-- DinaPlayer: hover tooltips on the control buttons. Stock hardcodes 16.
	tooltip_size = 16,
	tooltip_size_fullscreen = 0, -- 0 = inherit `tooltip_size`

	-- DinaPlayer: "Следующая серия" plate (elements/SkipEnding.lua). Title font
	-- size in pixels; the hint, padding and gap scale with it.
	skip_ending_size = 15,
	skip_ending_size_fullscreen = 0, -- 0 = inherit `skip_ending_size`
	...
	font_bold = false,
	autohide = false,
	menu_anchor = 'center', -- DinaPlayer: 'center' (stock) | 'bottom_right' (YouTube popup)
...
-- DinaPlayer patch: fullscreen-specific element sizes.
-- Returns `options.{name}_fullscreen` while fullscreen/maximized (if set above 0),
-- otherwise the plain `options.{name}`. Lets us keep a compact windowed UI while
-- rendering Netflix-sized controls/title/timeline in fullscreen, without touching
-- `scale_fullscreen` (which would blow up menus and popups too).
function fullscreen_size(name)
	local fs = options[name .. '_fullscreen']
	if state.fullormaxed and fs and fs > 0 then return fs end
	return options[name]
end
...
bind_command('subtitles', create_select_tracklist_type_menu_opener({
	...
	-- DinaPlayer: no "Load" item in the quick popup (kept in the main menu instead).
	-- "None" item at the top disables subtitles (clicking a track no longer toggles off).
	none_item = true,
}))
bind_command('audio', create_select_tracklist_type_menu_opener({
	-- DinaPlayer: no header in the audio popup, just the tracks — and only the
	-- language code on the right, like the subtitles popup, instead of a second
	-- line of codec/channels/sample rate nobody picks a track by.
	type = 'audio', prop = 'aid', lang_hint = true,
	max_visible = 8, -- DinaPlayer: scroll after 8 rows, like the other popups
}))
...
bind_command('playlist', create_self_updating_menu_opener({
	-- DinaPlayer: no header in the playlist popup, just the entries.
	type = 'playlist',
	list_prop = 'playlist',
	-- DinaPlayer: the right-side state icon is a clickable button (toggle_icon);
	-- always reserve its column so rows keep their size; refresh it when the
	-- watched sets change.
	icon_action = 'toggle_icon',
	reserve_icons = true,
	-- DinaPlayer: cap the popup at 9 visible rows before it scrolls (one taller
	-- than the other popups, which use 8).
	max_visible = 9,
	update_props = {'user-data/dina-watched', 'user-data/dina-inprogress', 'user-data/dina-pretty'},
	serializer = function(playlist)
		local items = {}
		local force_filename = mp.get_property_native('osd-playlist-entry') == 'filename'
		-- DinaPlayer: leading state button icon (from scripts/watched-tracker.lua):
		-- watched -> check, in progress -> clock, otherwise an empty circle. Clicking
		-- it toggles the saved "seen" state (see on_activate / dina-toggle-seen).
		local watched = mp.get_property_native('user-data/dina-watched', {})
		local in_progress = mp.get_property_native('user-data/dina-inprogress', {})
		-- DinaPlayer: readable names from scripts/pretty-title.lua, when it could
		-- make sense of the file name (otherwise we keep the name as it is).
		local pretty = mp.get_property_native('user-data/dina-pretty', {})
		for index, item in ipairs(playlist) do
			local title = type(item.title) == 'string' and #item.title > 0 and item.title or false
			-- DinaPlayer: show the file name without its extension (.filename), not .basename.
			-- DinaPlayer: prefer our cleaned name (dina-pretty) over mpv's own entry
			-- title so the playlist matches the top bar; mpv's title is the fallback
			-- (e.g. real titles for network streams whose file name is just a URL).
			local name = (not force_filename and pretty[item.filename])
				or (not force_filename and title)
				or (is_protocol(item.filename) and item.filename or serialize_path(item.filename).filename)
			-- DinaPlayer: leading state icon (from scripts/watched-tracker.lua):
			-- watched -> filled check box, in progress -> indeterminate (dash) box,
			-- not watched -> empty box outline. The "not watched" box is dimmed
			-- (icon_opacity) so it reads as a faint dark-grey placeholder and the
			-- watched/in-progress rows stand out. reserve_icons keeps the column +
			-- hover zone either way.
			local state_icon = 'check_box'
			local state_opacity = nil
			if not watched[item.filename] then
				if in_progress[item.filename] then
					state_icon = 'indeterminate_check_box'
				else
					state_icon = 'check_box_outline_blank'
					state_opacity = 0.4
				end
			end
			items[index] = {
				title = name,
				-- DinaPlayer: number rendered by Menu as a fixed-width left column
				-- (right-aligned), so every title starts at the same x.
				number = index .. '.',
				active = item.current,
				value = index,
				icon = state_icon,
				icon_opacity = state_opacity,
			}
		end
		return items
	end,
	on_activate = function(event)
		-- DinaPlayer: clicking the leading state icon flips the saved watched state
		-- (handled by scripts/watched-tracker.lua); a normal click plays the entry.
		-- Guard the empty-playlist placeholder row, whose value is the string
		-- 'ignore' rather than an index — the arithmetic below used to crash on it.
		local index = tonumber(event.value)
		if not index then return end
		if event.action == 'toggle_icon' then
			local fn = mp.get_property('playlist/' .. (index - 1) .. '/filename')
			if fn then mp.commandv('script-message', 'dina-toggle-seen', fn) end
			return
		end
...
local constructors = {
	...
	-- DinaPlayer: "next episode in N…" plate over the closing credits.
	skip_ending = require('elements/SkipEnding'),
}
