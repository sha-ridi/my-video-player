...
function Elements:update_proximities()
	-- DinaPlayer: `config.opacity.curtain = 0` means menus are popups, not modals —
	-- a curtain that draws nothing must not disable the elements underneath it.
	-- Mirrors the same guard in `Element:get_visibility()`; without it, elements
	-- below the curtain get their proximity reset on every cursor move while a
	-- popup is open, which hides the whole control bar mid-playback.
	local curtain_is_visible = Elements.curtain.opacity > 0 and config.opacity.curtain > 0
	local curtain_render_order = curtain_is_visible and Elements.curtain.render_order or 0
