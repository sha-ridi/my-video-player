	...
	for i, item in ipairs(items) do
		...
		elseif kind == 'command' then
			if #params ~= 2 then
				...
				-- DinaPlayer: which cluster this popup button sits in, so its hover
				-- tooltip can hide while its popup is open above it.
				local tooltip_group = ({
					['script-message settings-menu-open'] = 'right',
					['script-message dina-subtitles-open'] = 'right',
					['script-binding uosc/audio'] = 'right',
					['script-binding uosc/playlist'] = 'left',
					['script-binding uosc/prev'] = 'left',
					['script-binding uosc/next'] = 'left',
				})[params[2]]
				local element = Button:new('control_' .. i, {
					render_order = self.render_order,
					icon = params[1],
					icon_scale = ICON_SCALE[params[1]],
					tooltip_group = tooltip_group,
					-- DinaPlayer: let the button know which uosc menu it opens so it
					-- can render an "active" container while that popup is open.
					-- Verified live via IPC (user-data/uosc/menu/type) against this
					-- uosc build: the subtitles menu's type is 'sub', not 'subtitles'
					-- (it comes from create_select_tracklist_type_menu_opener's
					-- `type = 'sub'` in lib/menus.lua, matching track.type values).
					menu_type = ({
						['script-binding uosc/menu-blurred'] = 'menu',
						['script-binding uosc/menu'] = 'menu',
						['script-message settings-menu-open'] = 'settings',
						['script-binding uosc/playlist'] = 'playlist',
						['script-message dina-subtitles-open'] = 'sub',
		...
		elseif kind == 'cycle' then
			if #params ~= 3 then
				...
				local element = CycleButton:new('control_' .. i, {
					...
					states = states,
					tooltip = tooltip,
					-- DinaPlayer: fullscreen sits in the right cluster.
					tooltip_group = params[2] == 'fullscreen' and 'right' or nil,
				})
				table_assign(control, {element = element, sizing = 'static', scale = 1, ratio = 1})
				if badge then self:register_badge_updater(badge, element) end
			end
...
function Controls:update_dimensions()
	...
	local size = round(fullscreen_size('controls_size') * state.scale)
	local spacing = round(options.controls_spacing * state.scale)
	local margin = round(options.controls_margin * state.scale)
	-- DinaPlayer: the vertical gap from the icons down to the timeline is decoupled
	-- from `controls_margin` (which still sets the horizontal insets and the timeline's
	-- own gap to the screen edge), so the icons can sit closer to the slider than the
	-- slider sits to the bottom edge.
	local timeline_gap = round(2 * state.scale)
	...
	self.by = Elements:v('timeline', 'ay', display.height - window_border) - timeline_gap
