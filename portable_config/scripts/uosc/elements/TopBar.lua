...
function TopBar:init()
	...
	-- DinaPlayer: close gets a semi-transparent red hover chip; min/max use the
	-- default subtle white chip (see the chip rendering below).
	local close = {icon = 'close', hover_bg = '2311e8', hover_opacity = 0.5, command = function() mp.command('quit') end}
	-- DinaPlayer: crop_square reads heavier than the line/X icons, so render it a
	-- touch smaller (icon_scale) to look balanced.
	local max = {icon = 'crop_square', icon_scale = 0.82, command = maximized_command}
...
function TopBar:select_current_chapter()
	-- DinaPlayer: the top-bar current-chapter indicator is disabled — chapters
	-- live in the Settings menu instead. Keep current_chapter nil so nothing is
	-- rendered and no space/hitbox is reserved for it.
	if self.current_chapter ~= nil then
		self.current_chapter = nil
		self:update_dimensions()
	end
end
...
function TopBar:update_dimensions()
	self.size = round(fullscreen_size('top_bar_size') * state.scale)
...
-- DinaPlayer: the window control button (close/max/min) currently under the
-- cursor, if any. Lets a click on them while a menu is open both close the menu
-- and fire the action (see Menu:handle_cursor_down). Rect math mirrors render().
---@return {command: fun()}|nil
function TopBar:window_button_at_cursor()
	if not options.top_bar_controls or self:get_visibility() <= 0 then return nil end
	local ay, by = self.ay, self.ay + self.size
	local is_left = options.top_bar_controls == 'left'
	local button_ax = is_left and self.ax or (self.bx - self.size * #self.buttons)
	for _, button in ipairs(self.buttons) do
		local rect = {ax = button_ax, ay = ay, bx = button_ax + self.size, by = by}
		if get_point_to_rectangle_proximity(cursor, rect) <= 0 then return button end
...
function TopBar:render()
	...
	if options.top_bar_controls then
		...
		for _, button in ipairs(self.buttons) do
			...
			-- DinaPlayer: YouTube-like chip like the bottom bar — no background
			-- normally, a subtle semi-transparent chip on hover; icon stays white.
			-- close carries its own (red) hover_bg / stronger hover_opacity.
			local chip_color = button.hover_bg or fg
			local chip_opacity = is_hover and (button.hover_opacity or 0.12) or 0
			if chip_opacity > 0 then
				ass:rect(bg_ax, bg_ay, bg_bx, bg_by, {
					color = chip_color, opacity = visibility * chip_opacity, radius = state.radius,
				})
			end

			-- DinaPlayer: per-button icon_scale lets crop_square render a bit smaller.
			ass:icon(bg_ax + bg_size / 2, bg_ay + bg_size / 2, bg_size * 0.5 * (button.icon_scale or 1), button.icon, {
				color = fg,
				opacity = visibility,
			})
	...
	if main_title or state.has_playlist then
		...
		-- Playlist position
		-- DinaPlayer: hidden — the bottom-bar list button already opens the
		-- playlist, so this top-bar indicator/opener is redundant.
		if false then
		...
		if title_bx - title_ax > self.font_size * 3 and options.top_bar_title ~= 'no' then
			...
			if main_title then
				local opts = {
					...
					color = bgt,
					opacity = visibility,
					-- DinaPlayer: no text outline on the main title — plain white text.
					border = 0,
